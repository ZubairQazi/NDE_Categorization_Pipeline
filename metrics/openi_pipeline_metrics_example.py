...
async def run_pipeline_with_metrics(
    ...
    """Run the pipeline with simple metrics collection"""
    logger.info("Starting pipeline with OpenAI metrics collection...")
    ...
    try:
        ...
        # Start resource monitoring with simple callback
        def resource_callback(cpu_usage=None, memory_usage=None, **kwargs):
            if cpu_usage is not None and memory_usage is not None:
                metrics_collector.record_resource_usage(cpu_usage, memory_usage)
        ...
        if existing_batch_ids:
            ...
            if not dataset_path or not dataset_name:
                raise ValueError(
                    "Dataset path and name are required for normal processing"
                )
        ...
        # Display simple stats table
        stats = metrics_collector.get_simple_stats()
        ...
        # Ensure metrics collection is stopped even on error
        try:
            metrics_collector.stop_collection()
            resource_monitor.stop()

            # Still save partial metrics for debugging
            metrics_file = metrics_collector.save_metrics(
                f"{output_filename or 'error'}_partial_metrics.json"
            )
            logger.info(f"Partial metrics saved to {metrics_file}")

            # Show what we can from partial metrics
            if metrics_collector.metrics["total_samples"] > 0:
                stats = metrics_collector.get_simple_stats()
...
async def main():
    ...
    parser.add_argument(
        "--model", type=str, default="gpt-4.1-nano", help="Model to use"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        "--batch_file", type=str, help="Path to file containing batch IDs"
    )
    ...
    if has_batch_ids:
        ...
        print(
            "  python script.py --dataset_path <path> --dataset_name <name> [--output <name>] [--model <model>]"
        )
        print("\nUsage for batch processing:")
        print(
            "  python script.py --batch_ids <batch_id1> <batch_id2> ... [--output <name>] [--model <model>] [--mode <sync|batch>]"
        )
        print("  OR")
        print(
            "  python script.py --batch_file <path_to_batch_ids_file> [--output <name>] [--model <model>] [--mode <sync|batch>]"
        )
        sys.exit(1)
