...
class Normalizer(DataProcessor):
    ...
    def load_edam_topics(self) -> List[str]:
        ...
        return [
            topic[1:-1] if topic.startswith('"') and topic.endswith('"') else topic
    ...
    def load_synonyms(self) -> dict:
        ...
        edam["Synonyms"] = (
            edam["Synonyms"].fillna("").apply(lambda x: x.split("|") if x != "" else [])
        )
            ...
            for quoted_topic in self.quoted_topics:
                if quoted_topic.replace('"', "").lower() in cleaned_topics[i].lower():
                    cleaned_topics[i] = cleaned_topics[i].replace(
                        quoted_topic.replace('"', ""), quoted_topic
                    )
                    break
                else:
                    cleaned_topics[i] = cleaned_topics[i].replace('"', "")
        ...
        for prediction in predictions:
            ...
            for topic in self.quoted_topics:
                ...
                if formatted_topic in prediction:
                    processed_prediction = prediction.replace(
        ...
        for prediction in processed_predictions:
            if '"' in prediction:
                ...
                final_predictions.extend(
                    [pred.strip() for pred in prediction.split(",")]
                )
    ...
    def clean_and_split(self, predictions: List[str]) -> List[str]:
        ...
        for pred in predictions:
            ...
            cleaned_predictions.extend(
                [item.strip() for item in split_list if item.strip()]
            )

        # Remove 'Category X:' prefix if present
        cleaned_predictions = [
            re.sub(r"Category \d+:", "", pred) for pred in cleaned_predictions
        ]
        return cleaned_predictions

    def identify_unexpected_predictions(
            ...
            if result.model_response and result.categories:
                ...
                if len(predictions) <= 1:
                    ...
                    if (
                        '"' not in prediction
                        and " " in prediction
                        and prediction not in self.edam_topics
                    ):
            ...
            if result.model_response and "hallucinations" in result.model_response:
                ...
                for hallucination in hallucinations:
                    ...
                    sorted_topics = sorted(
                        self.edam_topics,
                        key=lambda topic: Levenshtein.distance(hallucination, topic),
                    )
                    ...
                    if not matched:
                        sorted_synonyms = sorted(
                            ...
                            key=lambda topic: Levenshtein.distance(
                                hallucination, topic
                            ),
                        )
        ...
        for result in results:
            if result.model_response and "hallucinations" in result.model_response:
                ...
                for hallucination in hallucinations:
                    if hallucination in matched_topics:
                        logger.info(
                            f"'{hallucination}' matches topic '{matched_topics[hallucination]}'"
                        )
            ...
            if result.categories:
                hallucinations = set(
                    [
                        ...
                        if cat.replace(".", "").replace('"', "") not in self.edam_topics
                    ]
                )
